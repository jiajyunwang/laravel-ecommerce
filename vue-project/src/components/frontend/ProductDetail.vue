...
async function fetchProduct() {
    try {
        const response = await axios.get(`/api/product-detail/${route.params.id}`)
...
async function addToCart() {
    if (cartSubmitting.value) return
        ...
        showUpperLimit.value = false
    try {
        const response = await axios.post('/request-action', {
...
function directBuy() {
    ...
    cartStore.requestAction = 'checkout'
    cartStore.quantity = quantity.value
}

function doSearch() {
    if (searchInput.value.trim()) {
        router.push({ path: '/search', query: { search: searchInput.value.trim() } })
    }
}
...
async function fetchReviews() {
    ...
    reviewPage.value++
    try {
        const response = await axios.get(`/api/reviews/fetch?page=${reviewPage.value}&sort_by=${reviewSortBy.value}&sort_order=${reviewSortOrder.value}&product_id=${route.params.id}`)
        const data = response.data
...
<template>
    ...
    <div class="product-description">
        ...
        <div class="description-body" v-html="sanitizedDescription"></div>
    </div>
    ...
    <div class="product-review">
        ...
        <div class="description-body">
            ...
            <div ref="loadingIndicatorRef" id="loading-indicator" :class="[{ loader: reviewLoading }]">
                <span v-if="reviewAllLoaded">無更多評價</span>
            </div>
        </div>
    </div>

    <div v-if="showCartSuccess" class="popup-content">
        <p>已成功加入購物車</p>
    </div>
