...
const checkIds = computed(() => {
    const requestAction = cartStore.requestAction
    let check = undefined
    if (requestAction === 'checkout') {
        check = cartStore.productId
        return check
    } else if (requestAction === 'cart-to-checkout') {
        check = cartStore.selectedIds
        return check
    }
    if (!check) return []
})
...
async function fetchCheckout() {
    ...
    try {
        let url = undefined
        let form = {}
        if (cartStore.requestAction === 'checkout') {
            url = '/request-action'
            form = {
                product_id: checkIds.value,
                requestAction: 'checkout',
                quantity: cartStore.quantity,
            }
        } else if (cartStore.requestAction === 'cart-to-checkout') {
            url = '/user/checkout'
            form = {
...
async function handleCheckout() {
    ...
    try {
        ...
        await axios.get('/sanctum/csrf-cookie');
        await axios.post('/user/order/store', body)    
